...
export class CreateGroupuserTable1710382600173 implements MigrationInterface {
    name = 'CreateGroupuserTable1710382600173'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`group_user\` (\`id\` int NOT NULL AUTO_INCREMENT, \`MaNV\` varchar(255) NOT NULL, \`image\` varchar(255) NOT NULL, \`GioiTinh\` varchar(255) NOT NULL, \`NamSinh\` datetime NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`status\` varchar(255) NOT NULL, \`userId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`account\` CHANGE \`reset_password_token\` \`reset_password_token\` varchar(255) NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`account\` CHANGE \`reset_password_token\` \`reset_password_token\` varchar(255) NULL DEFAULT 'NULL'`);
